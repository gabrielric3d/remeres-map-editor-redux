...
ToolOptionsSurface::ToolOptionsSurface(wxWindow* parent) : NanoVGCanvas(parent, wxID_ANY, wxWANTS_CHARS) {
	...
	// Removed Bind(wxEVT_PAINT...) as NanoVGCanvas handles it
	// Removed Bind(wxEVT_ERASE_BACKGROUND...)
...
void ToolOptionsSurface::OnNanoVGPaint(NVGcontext* vg, int width, int height) {
	// Background (handled by NanoVGCanvas or custom clear)
	nvgBeginPath(vg);
...
void ToolOptionsSurface::DrawToolIcon(NVGcontext* vg, const ToolRect& tr) {
	...
	if (is_selected) {
		// Accent color background for selected
		nvgFillColor(vg, nvgRGBA(0, 120, 215, 60)); // Selected blue tint
	} else if (is_hover) {
		nvgFillColor(vg, nvgRGBA(255, 255, 255, 20)); // Subtle hover
	} else {
		nvgFillColor(vg, nvgRGBA(60, 60, 65, 255)); // Standard dark card
	}
	nvgFill(vg);
	...
	if (tr.brush) {
		...
		if (tex > 0) {
			int iconSize = 28; // Slightly smaller than button
			...
			nvgText(vg, x + w/2, y + h/2, tr.tooltip.Left(1).c_str(), nullptr);
		}
	}
}
...
void ToolOptionsSurface::DrawSlider(NVGcontext* vg, const wxRect& rect, const wxString& label, int value, int min, int max, bool active) {
	...
	nvgText(vg, x, y + h/2, label.c_str(), nullptr);

	// Track
	float label_w = FromDIP(70);
	float track_x = x + label_w;
	float track_w = w - label_w - FromDIP(40);
	float track_h = 4.0f;
	...
	if (value > min && max > min) {
		...
		nvgFillColor(vg, nvgRGBA(0, 120, 215, 255)); // Accent
		nvgFill(vg);
...
void ToolOptionsSurface::DrawCheckbox(NVGcontext* vg, const wxRect& rect, const wxString& label, bool value, bool hover) {
	...
	nvgFillColor(vg, nvgRGBA(230, 230, 230, 255));
	nvgTextAlign(vg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
	nvgText(vg, x + box_sz + 8, y + h/2, label.c_str(), nullptr);
}
...
void ToolOptionsSurface::OnMouse(wxMouseEvent& evt) {
	...
	if (evt.Dragging() && evt.LeftIsDown()) {
		...
		// Thickness (stub implementation for now as it was in original)
