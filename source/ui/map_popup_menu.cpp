...
void MapPopupMenu::Update() {
	...
	if (anything_selected) {
		...
		if (editor.selection.size() == 1) {
			...
			if (target && (target->isBorder() || target->isGroundTile() || target->isWall() || target->isCarpet() || target->getDoodadBrush())) {
				Append(MAP_POPUP_MENU_OPEN_IN_BRUSHES_EDITOR, "Open in Brushes Editor", "Open this ground, border, wall, carpet or doodad in the Brushes Editor")->SetBitmap(IMAGE_MANAGER.GetBitmap(ICON_PAINTBRUSH, wxSize(16, 16)));
			}
		}
	}
