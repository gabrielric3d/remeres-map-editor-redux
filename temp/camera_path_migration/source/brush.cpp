...
void Brushes::init()
{
	...
	addBrush(g_gui.worldboss_brush = newd FlagBrush(TILESTATE_WORLDBOSS));
...
std::string FlagBrush::getName() const
{
	switch(flag) {
		...
		case TILESTATE_WORLDBOSS: return "World Boss Zone brush (0x40)";
	}
...
int FlagBrush::getLookID() const
{
	switch(flag) {
		...
		case TILESTATE_WORLDBOSS: return EDITOR_SPRITE_WORLDBOSS;
	}
	return 0;
}
