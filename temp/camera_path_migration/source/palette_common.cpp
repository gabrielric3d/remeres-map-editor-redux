...
BEGIN_EVENT_TABLE(BrushToolPanel, PalettePanel)
	...
	EVT_TOGGLEBUTTON(PALETTE_TERRAIN_WORLDBOSS_TOOL,BrushToolPanel::OnClickWorldBossZoneBrushButton)
END_EVENT_TABLE()
...
void BrushToolPanel::LoadAllContents()
{
	...
	if(large_icons) {
		...
		ASSERT(g_gui.worldboss_brush);
		sub_sizer->Add(worldbossBrushButton = newd BrushButton(this, g_gui.worldboss_brush, RENDER_SIZE_32x32, PALETTE_TERRAIN_WORLDBOSS_TOOL));
			worldbossBrushButton->SetToolTip("World Boss Zone Tool");
		...
		ASSERT(g_gui.worldboss_brush);
		sub_sizer->Add(worldbossBrushButton = newd BrushButton(this, g_gui.worldboss_brush, RENDER_SIZE_16x16, PALETTE_TERRAIN_WORLDBOSS_TOOL));
			worldbossBrushButton->SetToolTip("World Boss Zone Tool");
	}
...
void BrushToolPanel::DeselectAll()
{
	if(loaded) {
		...
		worldbossBrushButton->SetValue(false);
	}
}
...
Brush* BrushToolPanel::GetSelectedBrush() const
{
	...
	if(worldbossBrushButton->GetValue())
		return g_gui.worldboss_brush;
	return nullptr;
}
...
bool BrushToolPanel::SelectBrush(const Brush* whatbrush)
{
	...
	} else if(whatbrush == g_gui.worldboss_brush) {
		button = worldbossBrushButton;
	}
...
void BrushToolPanel::OnClickWorldBossZoneBrushButton(wxCommandEvent& event)
{
	g_gui.ActivatePalette(GetParentPalette());
