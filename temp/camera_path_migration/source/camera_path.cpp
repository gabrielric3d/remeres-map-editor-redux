...
void CameraPaths::applySnapshot(const CameraPathsSnapshot& snapshot)
{
	paths = snapshot.paths;
	active_path = snapshot.active_path;
	active_keyframe = snapshot.active_keyframe;
}

void CameraPaths::swapSnapshot(CameraPathsSnapshot& snapshot)
{
	std::swap(paths, snapshot.paths);
	std::swap(active_path, snapshot.active_path);
	std::swap(active_keyframe, snapshot.active_keyframe);
}
...
bool CameraPaths::loadFromFile(const FileName& mapFile, wxString* outError)
{
	...
	try {
		...
		for(const auto& pathNode : pathsNode) {
			...
			const nlohmann::json& keyframesNode = pathNode.value("keyframes", nlohmann::json::array());
			if(keyframesNode.is_array()) {
				for(const auto& keyNode : keyframesNode) {
					...
					key.pos.z = keyNode.value("z", rme::MapGroundLayer);
...
bool CameraPaths::saveToFile(const FileName& mapFile, wxString* outError) const
{
	...
	for(const CameraPath& path : paths) {
		...
		for(const CameraKeyframe& key : path.keyframes) {
