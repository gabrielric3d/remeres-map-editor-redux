...
#define String(key, dflt) \
	...
	Int(TRANSPARENT_GROUND_ITEMS, 0);
	Int(SHOW_ALL_FLOORS, 1);
	Int(SHOW_INGAME_BOX, 0);
	Int(CUSTOM_CLIENT_BOX, 0);
	Int(CLIENT_BOX_WIDTH, rme::ClientMapWidth);
	Int(CLIENT_BOX_HEIGHT, rme::ClientMapHeight);
	Int(CLIENT_BOX_OFFSET_X, 0);
	Int(CLIENT_BOX_OFFSET_Y, 2);
	Int(SHOW_LIGHTS, 1);
	Int(LIGHT_HOUR, 12);
	...
	Int(SHOW_NPC_PATHS, 1);
	Int(SHOW_CREATURE_WANDER_RADIUS, 0);
	Int(ANIMATE_CREATURE_WALK, 0);
	Int(SHOW_ONLY_GROUNDS, 0);
	Int(SHOW_CHUNK_BOUNDARIES, 0);
	Int(FLOOR_FADING, 1);
	Int(FLOOR_FADING_MODE, 0);
	Int(FLOOR_FADING_DURATION, 300);
	Int(FLOOR_FADING_EASING, 1);
	Int(FLOOR_FADING_OPACITY, 100);
	...
	Int(USE_LAZY_LOADING_SPRITES, 0);
	Int(USE_SPRITE_CACHE, 0);
	Int(SPRITE_CACHE_SIZE, 5000);
	Int(USE_DISK_SPRITE_CACHE, 1);
	Int(USE_OPTIMIZED_MAP_LOADING, 1);
	Int(USE_ANTIALIASING, 1);
